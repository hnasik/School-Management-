...
function renderClassCards(){
  ...
  extraClasses.forEach(c=>{
    ...
    card.dataset.cls = c.key;
    card.innerHTML = `<div class="num">${c.label}</div><div class="sub">Tap to view students</div>`;
    card.addEventListener("click", ()=> openStudentModal(c.key));
...
snap.forEach(d=>{
  const data = d.data();
  const sc = normalizeClass(data.studentClass || data.class || "");
  if(sc === wanted){
...
if(!docs.length){
  studentModalSub.textContent = `No students found for this class.`;
  return;
}

allStudentsForClass = docs;
filteredStudents = [...docs];
studentModalSub.textContent = `Found ${docs.length} student(s).`;
renderStudentPage();
...
function buildPdfTemplate(student, opts){
  ...
  const father = escapeHtml(
    student.fatherName || student.father || student.father_name || "-"
  );

  const dob = escapeHtml(
    student.dob || student.dateOfBirth || student.birthDate || "-"
  );

  const photo = student.photoUrl || null;

  const headerLogo = logoUrl
    ? `<img src="${logoUrl}" style="height:50px;margin-right:12px" />`
    : "";

  const photoHtml = photo
    ? `<img src="${photo}" style="width:130px;height:160px;object-fit:cover;border-radius:12px;border:3px solid #6366f1" />`
    : `<div style="width:130px;height:160px;border-radius:12px;border:2px dashed #aaa;display:flex;align-items:center;justify-content:center;color:#777">Photo</div>`;

  const infoBox = (label, value) => `
    <div style="background:#f9fafb;padding:10px 12px;border-radius:10px;border:1px solid #e5e7eb">
      <div style="font-size:11px;color:#6b7280">${label}</div>
      <div style="font-weight:600;color:#111;margin-top:2px">${value}</div>
    </div>
  `;

  const html = `
  <div style="font-family:Segoe UI,Arial;background:#eef2ff;padding:30px">
    <div style="max-width:900px;margin:auto;background:#fff;border-radius:18px;overflow:hidden;box-shadow:0 15px 40px rgba(0,0,0,0.15)">

      <!-- Header -->
      <div style="background:linear-gradient(135deg,#4f46e5,#6366f1);color:#fff;padding:22px 26px;display:flex;align-items:center">
        ${headerLogo}
        <div>
          <div style="font-size:22px;font-weight:800">${schoolName}</div>
          <div style="opacity:0.9;margin-top:4px">${exam} – Admit Card</div>
        </div>
        <div style="margin-left:auto;font-size:12px;opacity:0.85">
          Academic Year: ${escapeHtml(getSelectedAcademicYear())}
        </div>
      </div>

      <!-- Body -->
      <div style="padding:26px;display:flex;gap:26px">

        <!-- Left -->
        <div style="flex:1">
          <div style="font-size:21px;font-weight:700;color:#111">${studentName}</div>

          <div style="margin-top:16px;display:grid;grid-template-columns:1fr 1fr;gap:12px">
            ${infoBox("Roll No", roll)}
            ${infoBox("Class", cls)}
            ${infoBox("Father's Name", father)}
            ${infoBox("Date of Birth", dob)}
          </div>

          <div style="margin-top:22px;padding:14px 16px;background:#f8fafc;border-radius:12px;border-left:5px solid #6366f1">
        ...
        <div style="width:180px;text-align:center">
          ${photoHtml}
          <div style="margin-top:8px;font-size:12px;color:#666">Student Photo</div>

          <div style="margin-top:42px;border-top:1px solid #ccc;padding-top:6px;font-size:12px;color:#555">
      ...
      <!-- Footer -->
      <div style="padding:12px 26px;background:#f1f5f9;font-size:11px;color:#666;text-align:right">
