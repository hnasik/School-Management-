...
async function loadDashboard(user) {
  ...
  /* Class-wise count */
  /* Class-wise count (with Pre-primary classes) */
...
const classCount = {};
classOrder.forEach(c => classCount[c] = 0);

students.forEach(doc => {
  const c = doc.data().studentClass;

  if (classCount[c] !== undefined) {
    classCount[c]++;
  }
});
...
classOrder.forEach(c => {
  const label = classLabels[c] || `Class ${c}`;

  classWiseSummaryEl.innerHTML += `
    <div class="class-item">${label}: ${classCount[c]}</div>
  `;
});
