...
function renderClassCards() {
  ...
  extraClasses.forEach(c => {
    ...
    card.dataset.class = c.key;
    card.innerHTML = `<div class="num">${c.label}</div><div class="label">View students & manage delivery</div>`;
    card.addEventListener("click", () => openClassModal(c.key));
...
async function openClassModal(cls) {
  ...
  classModalSub.textContent = `Loading students...`;
  ...
  try {
    ...
    if (!user) {
      classModalSub.textContent = "Sign-in required.";
      return;
    }
    ...
    snap.forEach(doc => {
      const data = doc.data();
      const studentClass = normalizeClass(data.studentClass || data.class || "");
      if (studentClass === wanted) matched.push(doc);
    });

    if (matched.length === 0) {
      classModalSub.textContent = "No students found for this class.";
      return;
    }
    ...
    classModalSub.textContent = `Found ${currentStudents.length} student(s).`;

  } catch (err) {
    console.error("openClassModal error:", err);
    classModalSub.textContent = "Error loading students.";
  }
}
