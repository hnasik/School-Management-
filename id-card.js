...
titleText.textContent =
  (classLabelMap[cls] || `Class ${cls}`) + " Students";
...
function openGenerateModal(stu, row) {
  ...
  const displayClass = classLabelMap[stu.class] || `Class ${stu.class}`;
const classRollText = `Class: ${displayClass} | Roll: ${stu.roll || "-"}`;
...
function setupModalButtonHandlers() {
 ...
 const params = new URLSearchParams({
  ...
  window.open(
  "generate-id-card.html?" + params.toString(),
  "_blank"
);
