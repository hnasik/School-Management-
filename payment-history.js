...
extraClasses.forEach(c => {
  classSelect.innerHTML += `<option value="${c.value}">${c.label}</option>`;
});
...
async function loadStudents() {
  ...
  const selectedNorm = normalizeClass(classSelect.value);
  ...
  snap.forEach(doc => {
    const s = doc.data();
    const studentNorm = normalizeClass(s.studentClass);

    if (studentNorm == selectedNorm) {
      found = true;
      studentTableBody.innerHTML += `
        <tr>
          <td>${i++}</td>
